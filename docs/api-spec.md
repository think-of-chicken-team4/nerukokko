...
この文書は、**鶏ユニット（Raspberry Pi）・Webアプリとサーバー（Supabase）の間の通信の決まり**です。鶏・Webアプリ・サーバーはすべてこの仕様に合わせて実装します。
変えたいところがあれば、実装を変える前に PR でこの文書を更新し、影響を受ける担当者に知らせてください（`CLAUDE.md` §3）。
...
| 版 | 日付 | 内容 |
|---|---|---|
| v1.0 | 2026-09-24 | 初版。Webアプリ・バックエンド側で決定し、鶏・たまごもこれに合わせる |
